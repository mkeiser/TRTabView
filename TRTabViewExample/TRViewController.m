...
- (void)tabViewCommitTabAddition:(TRTabView *)tabView {
	...
	id modelObject = [NSString stringWithFormat:@"Tab %@", [NSNumber numberWithUnsignedInteger:self.globalTabCount++]];
...
- (NSMutableArray *)model {
	...
	if (!_model) {
		...
		for (NSUInteger i = 0; i < kNumberInitialTabs; i++) {
			...
			[_model addObject:[NSString stringWithFormat:@"Tab %@", [NSNumber numberWithUnsignedInteger:self.globalTabCount++]]];
		}
	}
